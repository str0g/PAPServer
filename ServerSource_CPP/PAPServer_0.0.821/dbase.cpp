...
void dBase::Close(){
    ...
    cerr<<"dbase connection closed"<<endl;
...
bool dBase::Search(const char *cSearch,bool Coma,char *cToSearch){
    ...
    if (Coma){
        sprintf(Query, "select * from %s where %s = '%s' and FileOwner != '%d'\
                ",p_cTableName,cToSearch,cSearch,getpid());
    }else{
        sprintf(Query, "select * from %s where %s = %s and FileOwner != '%d'\
                ",p_cTableName,cToSearch,cSearch,getpid());
    }
...
bool dBase::SearchFiles(XMLParser *p_xmlStructPointer,string *strctrl,const char *cSearch,bool Coma,char *cToSearch){
    ...
    */
    if(Search(cSearch,Coma,cToSearch)){
        pResult = mysql_store_result(pConnection);
        if(!p_xmlStructPointer->createXMLDoc("SharedFiles")){ return false; }
...
bool dBase::AskIfBanned(char *cSearch1,char *cSearch2,bool Coma,char *cToSearch1,char *cToSearch2){
    ...
    */
    ...
    if(Search2(cSearch1,cSearch2,Coma,cToSearch1,cToSearch2)){
        pResult = mysql_store_result(pConnection);
        while(Row = mysql_fetch_row(pResult)){
            k.string_naliczba<int>(Row[4]) == 1 ? bBanned = true : bBanned;
        }
        mysql_free_result(pResult);
    }
    return bBanned;
...
bool dBase::dbQuery(char *buf,string MsgOnSuccess){
    ...
    if(mysql_query(pConnection,buf) != 0){ //execute the query
        cerr<<buf<<endl;
        cerr<<"Error Executing the query: "<<mysql_error(pConnection)<<endl;
        return false;
    }
    cout<<MsgOnSuccess<<endl;
    return true;
}
